...
def test_c1_reports_a_pass_for_a_fitter_that_actually_reaches_the_target(monkeypatch):
    torch.manual_seed(0)
...
def test_c1_direction_is_FORWARD_because_ppisps_family_cannot_invert_itself(monkeypatch):
    """The defect this control had. PPISP's vignetting is clamped to [0,1], so it
    can only DARKEN; asking the fitter to UNDO a radial darkening asks for a
    radial brightening the per-camera stages cannot express, and the control
    reported `passed=False` for a fitter that works perfectly.

    Two assertions: the family really is darken-only, and the pair C1 builds runs
    render -> family(render) and not the other way.
    """
    from bench import lpips_attr as LA
    torch.manual_seed(4)
    vig = torch.tensor([[0.02, -0.01, -1.3, 0.25, 0.0]] * 3)
    f = LA.vig_falloff(LA.vig_uv(32, 40), vig)
    assert float(f.max()) <= 1.0 + 1e-6, "vignetting must be darken-only"
    assert float(f.min()) < 0.8, "and this fixture must actually darken, or it proves nothing"
    ...
    def cap(name, start, target, args, device='cpu'):
        ...
        return list(target), [{}]
    monkeypatch.setattr(CP, "run_fitter", cap)
    CP.synthetic_control("ppisp", renders, _l2_metric, _args(), "cpu")
    assert torch.equal(seen["start"], renders[0]), "C1 must start FROM the render"
    crf = LA.crf_identity_raw().expand(3, 4).clone() + 0.35
    assert torch.allclose(seen["target"], LA.apply_ppisp(renders[0], vig, crf), atol=1e-6), \
        "C1's target must be the render pushed FORWARD through the family"
